...
def aosring_randkeys(n=4):
	...
	# print('生成的私钥值为：', skeys)
	# print('生成的私钥个数为：', len(skeys))
	pkeys = [sbmul(sk) for sk in skeys]
	# print('生成的公钥值为：', pkeys)
	# print('生成的公钥个数为：', len(pkeys))
	i = randint(0, n-1)
	return pkeys, (pkeys[i], skeys[i])     #这里应该是返回生成的四个公私钥对，其中指定(pkeys[i], skeys[i])为签名者的公私钥对


def aosring_sign(pkeys, mypair, tees=None, alpha=None, message=None):
	assert len(pkeys) > 0
	message = message or hashpn(*pkeys)
	mypk, mysk = mypair # 这里对应的应该是(pkeys[i], skeys[i])
	myidx = pkeys.index(mypk)  # 这里指(pkeys[i], skeys[i])再pkeys里的索引值
